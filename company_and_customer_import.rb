...
CSV.foreach(company_csv_file, headers: true, :encoding => 'ISO-8859-1') do |row|
  ...
  begin
    ...
    if e.respond_to? :errors
      ...
      puts "Error creating company: CSV Row: #{x}, Company ID: #{row['id']} - #{e}"
      company_error_log.error "Error creating company: CSV Row: #{x}, Company ID: #{row['id']} - #{e}"
    end
  else
...
CSV.foreach(customer_csv_file, headers: true, :encoding => 'ISO-8859-1') do |row|
  ...
  begin
    ...
    if e.respond_to? :errors
      ...
      customer_error_log.error "Error creating customer - CSV Row: #{x}, Customer ID: #{row['id']}"
    else
      puts "Error creating customer: CSV Row: #{x}, Customer ID: #{row['id']} - #{e.errors}"
      customer_error_log.error "Error creating customer - CSV Row: #{x}, Customer ID: #{row['id']}"
    end
